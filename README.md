...
| 23 | [Interop, Username & Account Layers](#interop-username--account-layers) | Matrix interop, usernames, registration, cache-manager auth, anti-ban |
...
| 26 | [Condition of Use](#condition-of-use) | Channel follow agreement |
...
Groups can be tagged as AI groups (powered by WhatsApp's Meta AI) and read from the
socket afterwards.

```javascript
// Tag a group as an AI group
await sock.makeAIGroup(jid, true)

// Remove the AI tag
await sock.makeAIGroup(jid, false)

// Read the AI-group flag for a group
const isAI = await sock.isAIGroup(jid)

// Fetch metadata for all AI groups
const aiGroups = await sock.fetchAIGroupsMetadata()

// Set the AI group permission for a chat
await sock.setAIGroupPermission(jid, true)
```
...
```javascript
import { wrapSocket } from '@crysnovax/baileys'

// Wrap the raw socket to protect against spam-triggered bans
